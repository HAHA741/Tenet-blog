...
//	}
func (casbinService *CasbinService) Casbin() *casbin.SyncedEnforcer {
	log.Println("Entering Casbin method")

	var err error
	once.Do(func() {
		log.Println("Creating Gorm adapter")
		a, err := gormadapter.NewAdapterByDB(global.GVA_DB)
		if err != nil {
			log.Fatalf("Failed to create Gorm adapter: %v", err)
		}

		log.Println("Creating SyncedEnforcer")
		syncedEnforcer, err = casbin.NewSyncedEnforcer(global.GVA_CONFIG.Casbin.ModelPath, a)
		if err != nil {
			log.Fatalf("Failed to create SyncedEnforcer: %v", err)
		}
	})

	if syncedEnforcer == nil {
		log.Fatal("SyncedEnforcer is nil after initialization")
	} else {
		log.Println("SyncedEnforcer is not nil")
	}
	...
	log.Println("Returning SyncedEnforcer")
	return syncedEnforcer
}
